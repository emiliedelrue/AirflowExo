...
def sla_miss_callback(dag, task_list, blocking_task_list, slas, blocking_tis):
    logging.warning("=" * 60)
    logging.warning("[ALERTE SLA] Des tâches ont dépassé leur SLA !")
    for task in blocking_task_list:
        logging.warning(f"[ALERTE SLA] Tâche en retard : {task.task_id}")
    for sla in slas:
        logging.warning(
            f"[ALERTE SLA] SLA miss enregistré — "
            f"tâche : {sla.task_id} | "
            f"dag : {sla.dag_id} | "
            f"date d'exécution : {sla.execution_date}"
        )
    logging.warning("Simulation envoi notification équipe RTE...")
    logging.warning("[ALERTE SLA] Notification envoyée à : ops-rte@rte-france.com")
    logging.warning("=" * 60)
...
def verifier_apis(**context):
    apis = {
        ...
        "éCO2mix": (
            ...
            "/eco2mix-regional-cons-def/records?limit=1"
        ),
    }
    for nom, url in apis.items():
        try:
            ...
            if resp.status_code != 200:
                raise ValueError(f"API {nom} indisponible — status : {resp.status_code}")
    ...
    logging.info("Toutes les APIs sont disponibles.")
...
def collecter_meteo_regions(**context):
    ...
    for region, coords in REGIONS.items():
        ...
        logging.info(f"{region} — Ensoleillement : {ensoleillement_h:.2f}h | Vent : {vent_kmh:.1f} km/h")
    return resultats
...
def collecter_production_electrique(**context):
    ...
    params = {"limit": 100, "order_by": "date desc"}
    resp = requests.get(BASE_URL, params=params, timeout=15)
    resp.raise_for_status()
    ...
    for region in REGIONS:
        ...
        logging.info(f"{region} — Solaire : {production[region]['solaire_mw']} MW | Éolien : {production[region]['eolien_mw']} MW")
    return production
...
def analyser_correlation(**context):
    ...
    for region in REGIONS:
        ...
        if solaire > 0 and ensoleillement == 0:
            alertes_region.append(
                f"ANOMALIE DONNÉES : production solaire ({solaire:.0f} MW) sans ensoleillement"
            )
...
# --- Définition du DAG avec SLA ---
with DAG(
    dag_id="energie_meteo_dag_sla",
    default_args=default_args,
    description="Corrélation météo / production énergétique — RTE avec SLA",
    schedule="0 6 * * *",
